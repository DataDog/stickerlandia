...
switch (DrivingAdapterSettings.DrivingAdapter)
{
    ...
    case DrivingAdapter.AGNOSTIC:
        if (resources.PostgresResource is null)
        {
            throw new InvalidOperationException("PostgreSQL resource is required for AGNOSTIC driving adapter");
        }
        if (resources.MigrationResource is null)
        {
            throw new InvalidOperationException("Migration resource is required for AGNOSTIC driving adapter");
        }
        if (resources.KafkaResource is null)
        {
            throw new InvalidOperationException("Kafka resource is required for AGNOSTIC driving adapter");
        }
        builder.WithAgnosticApi(resources.PostgresResource, resources.KafkaResource, resources.MigrationResource, oidcServer);
        break;
...
// Setup DynamoDB tables when using AWS
if (resources.DynamoDbResource is not null)
{
